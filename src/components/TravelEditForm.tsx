...
import { X, MapPin, DollarSign, Upload, Loader2, Plus, Star, Trash2 } from 'lucide-react';
import { Travel } from '../types';

interface TravelEditFormProps {
  travel: Travel;
  user: { id: string; name: string; email: string };
  onSubmit: (id: string, travelData: Partial<Omit<Travel, 'id' | 'userId' | 'userName' | 'created_at'>>) => void;
...
export default function TravelEditForm({ travel, user, onSubmit, onDelete, onClose }: TravelEditFormProps) {
  ...
  useEffect(() => {
    ...
    setImages(travel.images || []);
    setCulturalSites(travel.cultural_sites || []);
    setPlacesToVisit(travel.places_to_visit || []);
    setRatings(travel.ratings || { mobility: 0, safety: 0, population: 0, vegetation: 0 });
  }, [travel]);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <Card className="max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <CardHeader className="flex items-center justify-between">
          <CardTitle>Редактировать путешествие</CardTitle>
